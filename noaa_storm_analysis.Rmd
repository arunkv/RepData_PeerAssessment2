...
# Fix typos is event names
cleanups <- c("AVALANCE", "AVALANCHE",
...
cleanups <- matrix(cleanups, nrow = length(cleanups)/2, ncol = 2, byrow = TRUE)
ignore <- apply(cleanups, 1, function(x) {
    data[data$evtype == x[1], "evtype"] <- x[2]    
})
...
_National Weather Service Instruction 10-1605(https://d396qusza40orc.cloudfront.net/repdata%2Fpeer2_doc%2Fpd01016005curr.pdf)_ 
from the NOAA. 

```{r prelim-anal, echo = FALSE, eval = TRUE, message = FALSE, warning = FALSE}
...
`r yearRange[1]` to `r yearRange[2]`. The data has been cleaned to adhere to
section 2.1 _Permitted Storm Data Events_ of the NOAA document. Around 
`r evtypeCount` event types are available in the data after cleanup.

# Results

## Storm Event Types and Population 

```{r analyze-by-pop, echo = FALSE, eval = TRUE, message = FALSE, warning = FALSE}
...
```{r plot-fatalities, echo = FALSE, eval = TRUE, message = FALSE, warning = FALSE}
# Plot fatalities
...
```{r plot-injuries, echo = FALSE, eval = TRUE, message = FALSE, warning = FALSE}
# Plot injuries
ggplot(topInjuries) +
    geom_bar(stat = "identity", 
             fill = "orangered1",
